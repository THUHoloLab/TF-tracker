...
def augment_scale(x, y, max_scale_percentage=10, scale_percentage=None):
    """
    Augment a target patch by scaling it. Scale percentage is uniformly sampled from [-MAX, MAX]
    :param x: (C, H, W) tensor of the event representation (patch)
    :param y: (2,) tensor of the gt displacement
    :param max_scale_percentage: int, max scale change (+/-) in percentage
    :return: x_aug, y_aug
    """
    _, patch_size_old_h, patch_size_old_w = x.shape  # 长方形的高度和宽度
    ...
    # 计算新的高和宽
    patch_size_new_h = int(round(patch_size_old_h * scaling))
    patch_size_new_w = int(round(patch_size_old_w * scaling))

    # 保证新的尺寸是奇数
    if patch_size_new_h % 2 == 0:
        patch_size_new_h += 1
    if patch_size_new_w % 2 == 0:
        patch_size_new_w += 1

    # 计算新的缩放比例
    scaling_h = patch_size_new_h / patch_size_old_h
    scaling_w = patch_size_new_w / patch_size_old_w

    # 对输入图像进行缩放
    x_aug = resize(x, [patch_size_new_h, patch_size_new_w], interpolation=InterpolationMode.NEAREST)

    # 如果图像缩小，进行零填充
    if scaling_h < 1.0 or scaling_w < 1.0:
        # 计算填充量
        padding_h = (patch_size_old_h - patch_size_new_h) // 2
        padding_w = (patch_size_old_w - patch_size_new_w) // 2
        if (patch_size_old_h - patch_size_new_h) % 2 != 0:
            padding_h += 1
        if (patch_size_old_w - patch_size_new_w) % 2 != 0:
            padding_w += 1

        x_aug = F.pad(x_aug, (padding_w, padding_w, padding_h, padding_h))
        x_aug = x_aug[
                :,
                0 : patch_size_old_h,
                0 : patch_size_old_w,
    ...
    # 如果图像放大，进行中心裁剪
    elif scaling_h > 1.0 or scaling_w > 1.0:
        # 确保裁剪后的尺寸完全与原始尺寸一致
        crop_h_start = (patch_size_new_h - patch_size_old_h) // 2
        crop_w_start = (patch_size_new_w - patch_size_old_w) // 2
        x_aug = x_aug[
                :,
                crop_h_start:crop_h_start + patch_size_old_h,
                crop_w_start:crop_w_start + patch_size_old_w,
                ]

    # 调整目标位移 y
    y_aug = y * torch.tensor([scaling_h, scaling_w])

    return x_aug, y_aug, (scaling_h, scaling_w)
